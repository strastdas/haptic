...
export function hideDotFiles<T extends { name?: string; children?: T[] }>(entries: T[]): T[] {
...
// Shortcut to string (platform-aware: ⌘ on macOS, ⌃ elsewhere)
...
/**
 * Escapes regex metacharacters in user input so it can be embedded in a RegExp
 * safely. Fixes the editor-search crash on inputs like `(`, `[`, `\`.
 */
