<script lang="ts">
	...
	import { collection, isDesktopApp, platform } from '@haptic/core/store';
	import { get } from 'svelte/store';

	let selected: 'notes' | 'daily' | 'tasks' | null = $state(null);

	function navigateTo(path: string) {
		// Desktop gates navigation on an opened collection (prompts cmd+o instead);
		// web keeps its historical direct navigation.
		if ($collection || !get(isDesktopApp)) {
			goto(path);
